...
function clearAnimationListener(animation: ExpandedAnimation, listener: android.view.animation.Animation.AnimationListener): void {
    if (!animation) {
        return;
    }
...
function clearEntry(entry: ExpandedEntry, removeListener: boolean): void {
    ...
    if (removeListener) {
        ...
        clearAnimationListener(entry.enterAnimation, listener);
        clearAnimationListener(entry.exitAnimation, listener);
        clearAnimationListener(entry.popEnterAnimation, listener);
        clearAnimationListener(entry.popExitAnimation, listener);
    }
}
...
function createDummyZeroDurationAnimation(): android.view.animation.Animation {
    const animation = new android.view.animation.AnimationSet(true);
